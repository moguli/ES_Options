...
void optionAdd(int Num,int Type,var StrikeOffs)
{
	CONTRACT* C = Combo+NCombos;
	CLots[NCombos++] = ifelse(Type&BUY,Num,-Num);
	C->Type = Type;
	C->Expiry = EXPIRY;
	C->fStrike = round(UnderL+StrikeOffs,0.5);
	C->fUnl = UnderL;
	
	var Premium = contractVal(C,UnderL,HistVol,DIVIDEND,RISKFREE);
	C->fAsk = C->fBid = Premium;
	if(Num) 
...
void main() 
{
	...
	if(!is(LOOKBACK)) 
	{
		...
		optionAdd(0,0,0);
		combo();
		//printf("\nMax Risk: %.2f",Multiplier*contractRisk(CLots[0],Combo+0,CLots[1],Combo+1,CLots[2],Combo+2,CLots[3],Combo+3));
		quit("Ok!");
	}
}	